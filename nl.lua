...
function NeverLose:CreateWindow(Config)
	...
	function Window:AddTab(Config)
		...
		function Tab:AddSection(Config)
			...
			local parent = (string.lower(Config.Position) == 'left' and LeftScroll) or RightScroll
			SectionFrame.Parent = parent
			SectionFrame.BackgroundTransparency = 1
			SectionFrame.ClipsDescendants = true
			SectionFrame.Size = UDim2.new(1, -5, 0, 0)
			SectionFrame.LayoutOrder = #parent:GetChildren() + 1  -- ИСПРАВЛЕНО: секции не накладываются
			...
			SectionList.Parent = SectionHandler
			SectionList.HorizontalAlignment = Enum.HorizontalAlignment.Center
			SectionList.SortOrder = Enum.SortOrder.LayoutOrder

			SectionList:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				if SectionList.AbsoluteContentSize.Y <= 1 then
					NeverLose.PlayAnimate(SectionFrame, VSlowTween, { Size = UDim2.new(1, -5, 0, 0) })
				else
					NeverLose.PlayAnimate(SectionFrame, VSlowTween, { Size = UDim2.new(1, -5, 0, SectionList.AbsoluteContentSize.Y + 19.5) })
				end
			end)

			local Section = {}

			function Section:AddLabel(Name, Warp)
				local BasedFrame = Instance.new("Frame")
				local BasedLabel = Instance.new("TextLabel")
				local LineFrame = Instance.new("Frame")
				local BasedHandler = Instance.new("Frame")
				local HandlerList = Instance.new("UIListLayout")
				local BasedCorner = Instance.new("UICorner")

				BasedFrame.Parent = SectionHandler
				...
				HandlerList.Parent = BasedHandler
				HandlerList.FillDirection = Enum.FillDirection.Horizontal
				HandlerList.HorizontalAlignment = Enum.HorizontalAlignment.Right
				HandlerList.SortOrder = Enum.SortOrder.LayoutOrder
				HandlerList.VerticalAlignment = Enum.VerticalAlignment.Center
				HandlerList.Padding = UDim.new(0, 5)

				BasedCorner.CornerRadius = UDim.new(0, 10)
				BasedCorner.Parent = BasedFrame

				if Warp then
					BasedFrame.Size = UDim2.new(1, 0, 0, 45)
					BasedLabel.Size = UDim2.new(1, -25, 0, 40)
					BasedLabel.TextWrapped = true
					BasedLabel.TextYAlignment = Enum.TextYAlignment.Top
				end

				local handle = { Root = BasedFrame }
				...
				function handle:AddToggle(Config)
					...
					local function updateUI()
						if toggleState then
							...
							NeverLose.PlayAnimate(Circle, SlowyTween, { BackgroundTransparency = 0, Position = UDim2.new(0.7, 0, 0.5, 0) })
						else
							NeverLose.PlayAnimate(Toggle, SlowyTween, { BackgroundColor3 = Color3.fromRGB(10, 13, 21) })
							NeverLose.PlayAnimate(Circle, SlowyTween, { BackgroundTransparency = 0.5, Position = UDim2.new(0.3, 0, 0.5, 0) })
						end
					end
					...
					local ToggleLib = {}
					function ToggleLib:GetValue() return toggleState end
					function ToggleLib:SetValue(v) toggleState = v; updateUI(); Config.Callback(v) end
				...
				function handle:AddButton(Config)
					...
					Config.Icon = Config.Icon or "chevron-large-right"
					Config.Callback = Config.Callback or function() end
					...
					BtnFrame.Parent = BasedHandler
					BtnFrame.BackgroundColor3 = Color3.fromRGB(26, 28, 36)
					BtnFrame.BorderSizePixel = 0
					BtnFrame.Size = UDim2.new(0, 30, 0, 18)
					...
					BtnIcon.FontFace = NeverLose.BuiltInBold
					BtnIcon.Text = Config.Icon
					BtnIcon.TextColor3 = Color3.fromRGB(223, 223, 223)
					BtnIcon.TextSize = 16
					BtnIcon.TextTransparency = 0.4

					NeverLose:CreateInput(BtnFrame, Config.Callback)
					return BtnFrame
				end
...
function NeverLose:CreateNotification()
	...
	NotificationList.Parent = Notification
	...
	function Notifier.new(Config)
		...
		Config.Content = Config.Content or "Hello"
		...
		Container.Size = UDim2.new(0, 0, 0, 55)
		...
		task.delay(Config.Duration, function()
			...
			NeverLose.PlayAnimate(NotifyStroke, SlowyTween, { Transparency = 1 })
...
function NeverLose:CreateLogger()
	...
	Log.Position = UDim2.new(0, 25, 0, 25)
	Log.Size = UDim2.new(0, 25, 0, 25)

	LogList.Parent = Log
	LogList.SortOrder = Enum.SortOrder.LayoutOrder
	LogList.Padding = UDim.new(0, 12)
	...
	function Logging.new(Icon, Message, Duration)
		...
		local LogLine = Instance.new("Frame")
		...
		LogFrame.Size = UDim2.new(0, 250, 0, 20)

		LogCorner.CornerRadius = UDim.new(0, 4)
		LogCorner.Parent = LogFrame

		LogStroke.Transparency = 0.65
		LogStroke.Color = Color3.fromRGB(45, 48, 58)
		...
		LogContent.Size = UDim2.new(1, -30, 0, 15)
		...
		LogLine.AnchorPoint = Vector2.new(0, 0.5)
		LogLine.BackgroundColor3 = NeverLose.AccentColor
		LogLine.BorderSizePixel = 0
		LogLine.Position = UDim2.new(0, -2, 0.5, 0)
		LogLine.Size = UDim2.new(0, 5, 1, 0)

		LogIcon.Parent = LogFrame
		LogIcon.BackgroundTransparency = 1
		LogIcon.Position = UDim2.new(0, 7, 0, 3)
		LogIcon.Size = UDim2.new(0, 15, 0, 15)
		LogIcon.FontFace = NeverLose.BuiltInBold
		LogIcon.Text = Icon
		LogIcon.TextColor3 = Color3.fromRGB(223, 223, 223)
		LogIcon.TextSize = 13
		LogIcon.TextTransparency = 0.25

		task.delay(Duration, function()
			NeverLose.PlayAnimate(LogFrame, SlowyTween, { BackgroundTransparency = 1 })
			NeverLose.PlayAnimate(LogStroke, SlowyTween, { Transparency = 1 })
			NeverLose.PlayAnimate(LogContent, SlowyTween, { TextTransparency = 1 })
			NeverLose.PlayAnimate(LogLine, SlowyTween, { BackgroundTransparency = 1 })
			NeverLose.PlayAnimate(LogIcon, SlowyTween, { TextTransparency = 1 })
			task.wait(0.3)
			LogFrame:Destroy()
		end)
	end
...
function NeverLose:CreateIndicator()
	...
	IndicatorList.Padding = UDim.new(0, 10)
	...
	function Indicators.new(Config)
		...
		Config.Icon = Config.Icon or 'crosshairs'
		Config.Color = Config.Color or 'Red'
		...
		IndicatorItem.BackgroundTransparency = 0.2
		IndicatorItem.BorderSizePixel = 0
		IndicatorItem.ClipsDescendants = true
		IndicatorItem.Size = UDim2.new(0, 100, 0, 40)

		ItemCorner.CornerRadius = UDim.new(0, 25)
		ItemCorner.Parent = IndicatorItem

		ItemLine.Parent = IndicatorItem
		ItemLine.AnchorPoint = Vector2.new(0, 0.5)
		ItemLine.BackgroundColor3 = Indicators.Color[Config.Color]
		ItemLine.BorderSizePixel = 0
		ItemLine.Position = UDim2.new(0, 2, 0.5, 0)
		ItemLine.Size = UDim2.new(0, 3, 0.65, 0)

		LineCorner.CornerRadius = UDim.new(0, 25)
		LineCorner.Parent = ItemLine

		ItemIcon.Parent = IndicatorItem
		ItemIcon.AnchorPoint = Vector2.new(0, 0.5)
		ItemIcon.BackgroundTransparency = 1
		ItemIcon.Position = UDim2.new(0, 10, 0.5, 0)
		ItemIcon.Size = UDim2.new(0, 25, 0, 25)
		ItemIcon.FontFace = NeverLose.BuiltInBold
		ItemIcon.Text = Config.Icon
		ItemIcon.TextColor3 = Indicators.Color[Config.Color]
		ItemIcon.TextSize = 21
		ItemIcon.TextTransparency = 0.25

		ItemContent.Parent = IndicatorItem
		ItemContent.AnchorPoint = Vector2.new(0, 0.5)
		ItemContent.BackgroundTransparency = 1
		ItemContent.Position = UDim2.new(0, 40, 0.5, 0)
		ItemContent.Size = UDim2.new(1, -40, 0, 25)
		ItemContent.Font = Enum.Font.GothamBold
		ItemContent.Text = Config.Name
		ItemContent.TextColor3 = Indicators.Color[Config.Color]
		ItemContent.TextSize = 20
		ItemContent.TextTransparency = 0.2
		ItemContent.TextXAlignment = Enum.TextXAlignment.Left

		local Indicator = { Visible = true, CurrentColor = Config.Color }
		...
		function Indicator:SetColor(c)
			Indicator.CurrentColor = c
			ItemLine.BackgroundColor3 = Indicators.Color[c]
			ItemIcon.TextColor3 = Indicators.Color[c]
			ItemContent.TextColor3 = Indicators.Color[c]
		end

		function Indicator:SetText(t)
			ItemContent.Text = t
		end
